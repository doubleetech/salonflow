    ...
    public static function summary(?int $branchId, string $startDate, string $endDate): array
{
    ...
    $sql = "SELECT
                ...
                -- Total Revenue = Cash + Transfer + POS (no tips)
                COALESCE(SUM(t.amount_cash + t.amount_transfer + t.amount_pos), 0) AS total_revenue,
                -- Cash total (no tips)
                COALESCE(SUM(t.amount_cash), 0) AS cash_total,
                -- Transfer total (no tips)
                COALESCE(SUM(t.amount_transfer), 0) AS transfer_total,
                -- POS total (no tips)
                COALESCE(SUM(t.amount_pos), 0) AS pos_total,
                -- Worker commissions (already calculated, just display)
 ...
 public static function branchBreakdown(string $startDate, string $endDate): array
{
    ...
    $stmt = $db->prepare(
        "SELECT
            b.id, b.name,
            COUNT(t.id) AS record_count,
            -- Revenue = Cash + Transfer + POS (no tips)
            COALESCE(SUM(t.amount_cash + t.amount_transfer + t.amount_pos), 0) AS revenue,
            -- Worker commissions (already calculated)
            COALESCE(SUM(t.worker_commission), 0) AS worker_commissions,
            -- Salon earnings (already calculated)
            COALESCE(SUM(t.salon_share), 0) AS salon_earnings
         FROM branches b
